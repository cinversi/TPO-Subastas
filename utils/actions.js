...
export const getSubastaReviews = async(id) => {
...
export const getIsFavorite = async(idSubasta) => {
...
export const deleteFavorite = async(idSubasta) => {
    const result = { statusResponse: true, error: null }
    try {
        const response = await db
            .collection("favorites")
    ...
    try {
        ...
        await Promise.all(
            map(response.docs, async(doc) => {
                ...
                const subasta = await getDocumentById("subastas", favorite.idSubasta)
                if (subasta.statusResponse) {
                    result.favorites.push(subasta.document)
                }
            })
        )
    } catch (error) {
...
export const getTopSubastas = async(limit) => {
    const result = { statusResponse: true, error: null, subastas: [] }
    try {
        const response = await db
            .collection("subastas")
            .orderBy("rating", "desc")
...
export const searchSubastas = async(criteria) => {
    const result = { statusResponse: true, error: null, subastas: [] }
    try {
        result.subastas = await fireSQL.query(`SELECT * FROM subastas WHERE name LIKE '${criteria}%'`)
    } catch (error) {
        result.statusResponse = false
        result.error = error
    }
...
export const getUsersFavorite = async(subastaId) => {
    const result = { statusResponse: true, error: null, users: [] }
    try {
        const response = await db.collection("favorites").where("idSubasta", "==", subastaId).get()
        await Promise.all(
            map(response.docs, async(doc) => {
                const favorite = doc.data()
